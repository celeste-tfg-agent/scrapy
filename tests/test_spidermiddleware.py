...
class TestProcessSpiderExceptionCalledOnce(TestSpiderMiddleware):
    """A process_spider_exception() method must be invoked exactly once for
    a given exception, even when the spider callback is a generator that
    yields output before raising, and the chain has a middleware defining
    process_spider_output positioned before the one defining
    process_spider_exception.

    Regression test for the bug where process_spider_exception() methods
    downstream of a process_spider_output-defining middleware were invoked
    more than once for the same exception, because the exception, once
    re-raised by _process_spider_exception() after being left unhandled,
    was caught again (and reprocessed) by each subsequent
    _evaluate_iterable() layer on its way out of the middleware chain.
    """

    @coroutine_test
    async def test_exception_handled_once_after_yield(self):
        ...
        class RecordExceptionMiddleware:
            def process_spider_exception(self, response, exception):
                ...
                return None  # don't handle it, let it propagate

        # RecordExceptionMiddleware is added first and
        # PassthroughOutputMiddleware second: since _add_middleware()
        # inserts each middleware's methods with appendleft(), the
        # middleware added last ends up first in the chain. This places
        # PassthroughOutputMiddleware's process_spider_output before
        # RecordExceptionMiddleware's process_spider_exception, which is
        # the ordering that triggers the bug.
        self.mwman._add_middleware(RecordExceptionMiddleware())
        self.mwman._add_middleware(PassthroughOutputMiddleware())
        ...
        it = await self.mwman.scrape_response_async(
            scrape_func, self.response, self.request
        )
    ...
    @coroutine_test
    async def test_exception_handled_once_without_yield(self):
        """Same as above but the callback raises immediately (still a
        generator function, just one that doesn't reach any yield). This
        must behave identically to the case where output was yielded
        first.
        """
        calls: list[Exception] = []
